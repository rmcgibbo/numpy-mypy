...
def _InferNdimsFromShape(funcname: str, return_type_args, bound_args: Dict[str, BoundArgument], ctx: FunctionContext):
    ...
    matches_ShapeType = [f for f in bound_args.values() if f is not None and is_same_type(f.formal_typ, ShapeType)]
...
def _InferNdimsReduction(funcname: str, return_type_args, bound_args: Dict[str, BoundArgument], ctx: FunctionContext):
    ...
    matches_AxesType = [f for f in bound_args.values() if f is not None and is_same_type(f.formal_typ, AxesType)]
    ...
    if isinstance(index_arg_typ, Instance):
        ...
        for arg in index_arg_typ.items:
            if isinstance(arg, Instance) and arg.type.name() == 'int':
                ...
                # Tuple[numpy.ndarray[builtins.int*, numpy.OneD], numpy.ndarray[builtins.int*, numpy.OneD]]
...
class NumpyPlugin(Plugin):
    ...
    def do_setup(self, ctx: FunctionContext):
        global ShapeType, AxesType
        self.api = ctx.api
        self.npmodule = ctx.api.modules['numpy']

        ShapeType = ctx.api.modules['numpy'].names['ShapeType'].type
        AxesType = ctx.api.modules['numpy'].names['AxesType'].type
